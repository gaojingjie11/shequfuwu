    ...
    <div class="container">
      ...
      <el-alert type="success" show-icon :closable="false" class="summary-alert">
        <template #title>
          ...
          支付时将按 {{ GREEN_POINTS_PER_YUAN }} 积分 = 1 元 自动优先扣除积分，不足部分再扣除余额。
        </template>
      </el-alert>
...
async function handlePay(fee) {
  ...
  await ElMessageBox.confirm(
    `本次将按 ${GREEN_POINTS_PER_YUAN} 积分=1元优先扣除 ${preview.points} 积分，余额扣除 ¥${formatAmount(preview.balance)}，确认支付吗？`,
    '混合支付确认',
    { type: 'warning' }
  )
  ...
  try {
    const result = await payPropertyFee(fee.id)
